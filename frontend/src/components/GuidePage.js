    ...
    return (
        <Typography component="div">
            ...
            <Box fontSize="body1.fontSize" m={1}>
                ...
                <p>这是 Rule 语法，方括号表示一个判断语句，紧跟在冒号后的是“那么”，感叹号后的是“否则”。它的基本形式是 <code>[ a ?? b ] : then ! else ;</code>，其中 <code>a</code> 就是检测对象的内部名称（见上表），<code>??</code> 代表运算符，目前所有支持的运算符有：{['==', '!=', '<', '>', '<=', '>=', '<-'].map((opt) => (<React.Fragment key={opt}><code>{opt}</code>&nbsp;</React.Fragment>))}<code>~</code>，其中 <code>{"<-"}</code> 和 <code>~</code> 分别代表“包含”和“符合（正则表达式）”。</p>
